...
data <- read.xlsx("AmazonGo.xlsx")

# remove words and process documents
#remove.words <- c("item", "one", "like", "get")
remove.words <- c("store", "amazon", "item", "shop", "translated", "by","google")
...
save(sentiment_score, file = "./sentiment_score.RData")
...
save(stm_model, file = "AmazonGo.RData")
...
save(k_selection, file = "./k_selection.RData")
plot(k_selection)
...
# Save Workbook
saveWorkbook(wb, file = "AmazonGo.xlsx", overwrite = T)