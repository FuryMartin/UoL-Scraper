...
def convert():
    ...
    with open('Mobile.csv','w',encoding='utf-8') as f:
        with open('NotMobile.csv','w',encoding='utf-8') as nf:
            ...
            nf.write('"","ReviewerName","ReviewerBadge","Grades","RatingDate","VisitDate","Likes","ViaMobiles","Title","documents","Restaurant"\n')
            #f.write('"","documents"\n')
            #nf.write('"","documents"\n')
                ...
                if "json" in filename:
                    ...
                    for t in data:
                        if t['ViaMobiles'] == "True":
                            ...
                            text = '"' + filename.strip(".json") + '","'+t['ReviewerName']+'","'+t['ReviewerBadge']+'",'+t['Grades']+',"'+t['RatingDate']+'","'+t['VisitDate'] + '",' + t['Likes'].strip(' ')+',"'+t['ViaMobiles']+'","' + t['Title'].replace('"', '')+'","' + t['Comment'].replace('"', '') + '"\n'
                            mobile_count += 1
                            f.write(text)
