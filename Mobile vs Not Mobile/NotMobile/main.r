...
data <- read.xlsx("NotMobile.xlsx")
date_paser <- "%B%d,%Y"
date_list <- out_data$meta$RatingDate
stop_words <- c("food", "wine", "restaur", "meal")
...
save(stm_model, file = "./RData/NotMobile.RData")
...
# Open Workbook
wb <- loadWorkbook("NotMobile.xlsx")
...
# Save Workbook
saveWorkbook(wb, file = "NotMobile.xlsx", overwrite = T)